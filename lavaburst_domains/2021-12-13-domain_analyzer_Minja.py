...
def statDomains(domains):
    results_file = "results_new/" + os.path.basename(domains_file)+"_stats.txt"
    ...
    plt.tight_layout()
    plt.savefig("results_new/" + os.path.basename(domains_file) + ".sizedist.png")
    plt.clf()
...
def get_insulation_of_domain_boundaries(domains, offset = 1, dist = 3):
    ...
    #plt.savefig("results_new/"+os.path.basename(hic_file)+os.path.basename(domains_file)+".png")
    return domains
...
def plot_E1_from_domains_size_dependence(E1, length_bin = 25000, maxlength = 400000):
    ...
    figure_path = "results_new2/"+os.path.basename(domains_file)+".E1_vs_size.png"
    ...
    print ("Overall E1 distribution")
    print (E1["E1"].max(),E1["E1"].min(),E1["E1"].median())
    print ("Number of TADs belonging to each size-category:")
    print (E1["domain_length"].value_counts().sort_index())
    print("Plotting")
...
def E1_near_boundaries_dendro(domains,boundaries,
                              separate_boundaries,
                              averaged = None,
                              fig_suffix = "_E1dendro.png"):
    ######################## dendrogramm #################
    fig_path = "results_new/"+os.path.basename(domains_file)+fig_suffix
...
def E1_vs_insulation_scatterplor(E1, E1_resolution, k, averaged, boundary_index, domains):
    ...
    figure_path = "results_new/" + os.path.basename(domains_file) + ".Insulation_violinplot.png"
    if os.path.isfile(figure_path) and not redraw_figs:
        return
...
def compartments_switch_at_domains_boundaries(domains, E1, E1_resolutoin,
    ...
    hashfile = os.path.join("hashedData",
                            ...
                            "."+compartments_switch_at_domains_boundaries.__name__+".v3.dump")
    print (hashfile)
    ...
    bad_boundaries_ids = np.concatenate((mask_l,mask_r))
    print (np.sum(bad_boundaries_ids))

    E1_near_boundaries_dendro(domains=domains,boundaries=boundaries[bad_boundaries_ids,:],
                              separate_boundaries=True,averaged=averaged[bad_boundaries_ids,:],
                              fig_suffix='bad_boundaries_dendro.png')

    E1_near_boundaries_dendro(domains=domains,boundaries=boundaries[~bad_boundaries_ids,:],
                              separate_boundaries=True,averaged=averaged[~bad_boundaries_ids,:],
                              fig_suffix='good_boundaries_dendro.png')
    ...
    basename ="results_new/"+os.path.basename(domains_file)
...
datasets = {
    "AatrE3_V4.TADs.5000.edited.2D":{"scores":"https://genedev.bionet.nsc.ru/site/hic_out/Anopheles/hicExplorer_TADs/results/AatrE3_V4.1000.hic_5000.h5.delt.0.05_score.bedgraph",
                                     "title":"An. atroparvus",
                                     "E1":"genedev_data/genedev.bionet.nsc.ru/site/hic_out/by_Species/Invertebrates/Anopheles/ActualData/eigs/AatrE3_V4/AatrE3_V4.ce.pc1.25000.eig.bedGraph"},
    "AalbS2_V4.TADs.5000.edited.2D":{"scores":"genedev_data/genedev.bionet.nsc.ru/site/hic_out/by_Species/Invertebrates/Anopheles/ActualData/domains/AalbS2_V4.h5.delt.0.05_score.bedgraph",
                                     "title":"An. albimanus",
                                      "E1":"genedev_data/genedev.bionet.nsc.ru/site/hic_out/by_Species/Invertebrates/Anopheles/ActualData/eigs/AalbS2_V4/AalbS2_V4.ce.pc1.25000.eig.bedGraph"},
    "AsteI2_V4.TADs.5000.edited.2D":{"scores":"https://genedev.bionet.nsc.ru/site/hic_out/Anopheles/hicExplorer_TADs/results/AsteI2_V4.1000.hic_5000.h5.delt.0.05_score.bedgraph",
                                     "title":"An. stephensi",
                                      "E1":"genedev_data/genedev.bionet.nsc.ru/site/hic_out/by_Species/Invertebrates/Anopheles/ActualData/eigs/AsteI2_V4/AsteI2_V4.ce.pc1.25000.eig.bedGraph"},
    "AmerR4_V4.TADs.5000.edited.2D":
                                    {"scores":"https://genedev.bionet.nsc.ru/site/hic_out/Anopheles/hicExplorer_TADs/results/AmerR4_V4.hic_5000.h5.delt.0.05_score.bedgraph",
                                     "title":"An. merus",
                                      "E1":"genedev_data/genedev.bionet.nsc.ru/site/hic_out/by_Species/Invertebrates/Anopheles/ActualData/eigs/AmerR4_V4/AmerR4_V4.ce.pc1.25000.eig.bedGraph"
                                      },
    "AcolNg_V4.TADs.5000.edited.2D": {
        ...
        "title": "An. colluzii",
        "E1": "genedev_data/genedev.bionet.nsc.ru/site/hic_out/by_Species/Invertebrates/Anopheles/ActualData/eigs/AcolNg_V4/AcolNg_V4.ce.pc1.25000.eig.bedGraph"
        }
}
...
for sp in datasets.keys():
    ...
    domains_file = "genedev_data/genedev.bionet.nsc.ru/site/hic_out/by_Species/Invertebrates/Anopheles/ActualData/domains/2D_edited/"+sp
    ...
    print ("Total number of TADs: ",len(domains_file))
    print ("By chromosome distribution:")
    print (domains["chr"].value_counts().sort_index())

    # debug!!!!
    # domains["shifts"] = domains.apply(lambda x: ((x["end"]-x["start"])//(2*domains_resolution))*domains_resolution ,
    #                                axis="columns")
    # domains.start = domains.apply(lambda x: x["start"] + x["shifts"],
    #                                axis="columns")
    # domains.end = domains.apply(lambda x: x["end"] + x["shifts"],
    #                                axis="columns")
